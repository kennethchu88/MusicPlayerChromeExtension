...
var model = (function () {
    ...
    chrome.extension.onMessage.addListener(function (request, sender, sendResponse) {
        ...
        switch (request.message) {
            case "load":
                ...
                sendResponse({ message: "player not ready" });
                break;
            case "play":
    ...
    // tells playlist.js to update its view
    var refreshPlayButtons = function () {
        // check that the playlist is open
        if (frontendPort) frontendPort.postMessage({ message: "refresh" });
    };
